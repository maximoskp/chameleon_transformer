...
from transformer.layers import EncoderLayer, DecoderLayer, DecoderOnlyLayer
...
class DecoderOnlyModel(nn.Module):
    ...
    def generate_mask(self, tgt, pad_token_idx=0):
        tgt_mask = (tgt != pad_token_idx).unsqueeze(1).unsqueeze(3).to(dev)
        seq_length = tgt.size(1)
        nopeak_mask = (1 - torch.triu(torch.ones(1, seq_length, seq_length), diagonal=1)).bool().to(dev)
        tgt_mask = tgt_mask & nopeak_mask
    ...
    def forward(self, tgt, pad_token_idx=0):
        tgt_mask = self.generate_mask(tgt, pad_token_idx)
        tgt_embedded = self.dropout(self.positional_encoding(self.decoder_embedding(tgt)))
        dec_output = tgt_embedded
        for dec_layer in self.decoder_layers:
...
class ContinuousEncoderOnlyWrapper(nn.Module):
    ...
    def forward(self, input_ids, pad_token_idx=0):
        output = self.encoder_model(input_ids)
        logits_vocab_size = self.decoder(output)
        return logits_vocab_size
    # end forward
...
class ContinuousDecoderModel(nn.Module):
    def __init__(self, tgt_vocab_size, d_model, num_heads, num_layers, d_ff, max_seq_length, dropout):
    ...
    def generate_mask(self, src, tgt):
        src_mask = (torch.sum(src, dim=2) != 0).unsqueeze(1).unsqueeze(2).to(dev)
        tgt_mask = (torch.sum(tgt, dim=2) != 0).unsqueeze(1).unsqueeze(3).to(dev)
        seq_length = tgt.size(1)
        nopeak_mask = (1 - torch.triu(torch.ones(1, seq_length, seq_length), diagonal=1)).bool().to(dev)
        tgt_mask = tgt_mask & nopeak_mask
        return src_mask, tgt_mask
    ...
    def forward(self, enc_output, src, tgt, pad_token_idx=0):
        ...
        return output
    # end forward
# end class ContinuousDecoderModel

class ContinuousDecoderOnlyModel(nn.Module):
    def __init__(self, tgt_vocab_size, d_model, num_heads, num_layers, d_ff, max_seq_length, dropout):
        super(ContinuousDecoderOnlyModel, self).__init__()
        self.decoder_embedding = nn.Linear(tgt_vocab_size, d_model)
        self.positional_encoding = PositionalEncoding(d_model, max_seq_length)
        self.decoder_layers = nn.ModuleList([DecoderOnlyLayer(d_model, num_heads, d_ff, dropout) for _ in range(num_layers)])
        self.fc = nn.Linear(d_model, tgt_vocab_size)
    ...
    def forward(self, tgt, pad_token_idx=0):
        ...
        return output
    # end forward
# end class ContinuousDecoderOnlyModel